...
export async function GET(
  ...
  if (!proposal?.signed_by_client_at) {
    return new NextResponse('PDF do cliente não encontrado.', { status: 404 })
  }
  ...
  const filename = `proposta-${id.slice(0, 8)}-cliente-assinado.pdf`
